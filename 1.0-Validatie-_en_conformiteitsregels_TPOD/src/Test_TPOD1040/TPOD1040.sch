...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:pattern id="TPOD_0990">
        <sch:rule context="//tekst:Kop[string(tekst:Opschrift) = 'Meet- en rekenbepalingen']">
            <sch:let name="APPLICABLE"
                value="$SOORT_REGELING = $OP or $SOORT_REGELING = $OV or $SOORT_REGELING = $WV"/>
            <sch:let name="fouten" value="foo:inleidendezinMeetRegelBepalingTPOD0980(..)"/>
            <sch:let name="CONDITION"
                value="string-length($fouten)=0"/>
            <sch:assert test="($APPLICABLE and $CONDITION) or not($APPLICABLE)"> 
                TPOD_0990: Het artikel Meet- en rekenbepalingen dan wel de specifieke bijlage met meet- en rekenbepalingen moet beginnen met een introducerende zin. Dit is niet zo in: <sch:value-of select="$fouten"/> </sch:assert>
        </sch:rule>
    </sch:pattern>

    <xsl:function name="foo:inleidendezinMeetRegelBepalingTPOD0980">
