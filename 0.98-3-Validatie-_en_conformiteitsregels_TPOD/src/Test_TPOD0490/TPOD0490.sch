    ...
    <sch:pattern id="TDOP_0490">
        <sch:rule context="//tekst:Lichaam/tekst:Hoofdstuk">
            ...
            <sch:assert test="($APPLICABLE and $CONDITION) or not($APPLICABLE)"> 
                TDOP_0490: Achter het cijfer van een titelnummer mag geen punt worden opgenomen. (betreft hoofdstukken, titels):  <xsl:value-of select="$hoofdstuk"/>: <xsl:value-of select="substring($fouten,1,string-length($fouten)-2)"/></sch:assert>
        </sch:rule>
    </sch:pattern>
