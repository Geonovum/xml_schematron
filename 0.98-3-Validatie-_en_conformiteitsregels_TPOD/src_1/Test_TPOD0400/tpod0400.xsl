...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   <!--PATTERN TDOP_0400-->
   <!--RULE -->
   <xsl:template match="//tekst:Kop" priority="1000" mode="M25">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="//tekst:Kop"/>
      ...
      <xsl:variable name="CONDITION">
         <xsl:value-of xmlns:sch="http://purl.oclc.org/dsdl/schematron"
                       ...
                       xmlns:gml="http://www.opengis.net/gml/3.2"
                       xmlns:lvbb="http://www.overheid.nl/2017/lvbb"
                       select="tekst:Label and tekst:Opschrift and tekst:Nummer"/>
      </xsl:variable>
      <!--ASSERT -->
      <xsl:choose>
         ...
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
               ...
               <svrl:text> TDOP_0400: Betreft
                (Label, Opschrift, Nummer): "<xsl:text/>
                  <xsl:value-of select="tekst:Label"/>
                  <xsl:text/>", "<xsl:text/>
                  <xsl:value-of select="tekst:Nummer"/>
                  <xsl:text/>", "<xsl:text/>
                  <xsl:value-of select="tekst:Opschrift"/>
                  <xsl:text/>": Een Kop
                moet bevatten een Label, een Nummer en een Opschrift. </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
