...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:let name="SOORT_REGELING">
        <xsl:value-of select="$xmlDocuments//stop:RegelingVersieInformatie/data:RegelingMetadata/data:soortRegeling/text()"/>
    </sch:let>
    ...
    <sch:pattern id="TDOP_0400">
        <sch:rule context="//tekst:Hoofdstuk/tekst:Kop[lower-case(tekst:Label) ne 'hoofdstuk']">
            <sch:let name="APPLICABLE">
                <xsl:value-of select="$SOORT_REGELING = $OP or $SOORT_REGELING = $OV or $SOORT_REGELING = $WV"/>
            </sch:let>
            <sch:let name="CONDITION" value="false()"/>
            <sch:assert test="$CONDITION"> 
                TDOP_0410: Een Hoofdstuk moet worden geduid met de label Hoofdstuk. Betreft label: 
                <sch:value-of select="$APPLICABLE"/><sch:value-of select="$SOORT_REGELING"/><sch:value-of select="tekst:Label"/> </sch:assert>
        </sch:rule>
    </sch:pattern>
